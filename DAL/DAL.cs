    ...
    public class DAL : Interfaces.IFunctionalities
    {
        ...
        public ObservableCollection<IceCream> GetAllIceCreams(Func<IceCream, bool> filter = null)
        {
            using (var db = new DALContext())
            {
                ...
                var newList = new List<IceCream>();

                foreach (var item in list.GroupBy(i => i.Name))
                {
                    var itemToAdd = item.OrderBy(i => i.ID)
                        .FirstOrDefault();
                    newList.Add(itemToAdd);
                }

                return new ObservableCollection<IceCream>(newList.Where(i => i.Name != null));
            }
        }
        ...
        public ObservableCollection<Shop> GetAllShops(Func<Shop, bool> filter = null)
        {
            using (var db = new DALContext())
            {
                ...
                var newList = new List<Shop>();

                foreach (var item in list.GroupBy(i => i.Name))
                {
                    var itemToAdd = item.OrderByDescending(i => i.ID)
                        .FirstOrDefault();
                    newList.Add(itemToAdd);
                } 

                return new ObservableCollection<Shop>(newList.Where(i => i.Name != null));
            }
        }
